# Module Name: cdi_module

This module provides CDI controller functionality.
...
- [CDI Management Server Setup](#cdi-management-server-setup)
- [License]("#license)
...
2.  **Clone the repository:**
    ...
    cd dci_physical_infrastructure/cdi
    ```
...
4.  **Build the module:**
    ...
    ```bash
    go build -o cdi_module cmd/server/main.go
    ```
...
3.  **Generate cdi_module protobuf code:**
    ...
    *   `./api/proto/cdi_interface.proto`: The path to the `.proto` file.

    This command generates the Go code, gRPC stubs, and validation code based on the `cdi_interface.proto` file. The output files will be placed in the same directory as the `cdi_interface.proto` file (specified by the `.` in the `--go_out`, `--go-grpc_out`, and `--validate_out` options, combined with the path to the `.proto` file).
...
## CDI Management Server Setup

Before running this module, ensure the following setup is completed on the target CDI management server. This involves creating symbolic links to the CDI control command binaries *directories*. 

1.  **Create Symbolic Links:** Create symbolic links with the following targets:

    ```bash
    ln -s <path_to_cdi_command_directory> /home/cdiadmin/bin
    ```

    Replace `<path_to_cdi_command_directory>` with the actual path to the directory you want to link *from*. 

2.  **Install expect command:** Install expect command using the following instructions:
