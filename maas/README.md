# Module Name: maas_module

This module provides MaaS controller functionality.
...
2.  **Clone the repository:**
    ...
    cd dci_physical_infrastructure/maas
    ```
...
4.  **Build the module:**
    ...
    ```bash
    go build -o maas_module cmd/server/main.go
    ```
...
3.  **Generate maas_module protobuf code:**
    ...
    protoc --proto_path=. \
        ...
        ./api/proto/maas_interface.proto
    ```
    ...
    *   `./api/proto/maas_interface.proto`: The path to the `.proto` file.

    This command generates the Go code, gRPC stubs, and validation code based on the `maas_interface.proto` file. The output files will be placed in the same directory as the `maas_interface.proto` file (specified by the `.` in the `--go_out`, `--go-grpc_out`, and `--validate_out` options, combined with the path to the `.proto` file).
