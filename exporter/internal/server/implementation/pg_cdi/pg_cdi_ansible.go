...
func (l PgCDIAnsibleImple) CmdExecute(ctx context.Context, remoteHost string, remotUser string, sshPrivateKeyFile string, playbook string, extrArgs string) (errMsg *string, jsonData map[string]interface{}) {
	defer func() {
	...
	if extrArgs != "" {
		l.Logger.V(2).Info("branch: extra arguments provided", "extraArgs", extrArgs)
...
func (l PgCDIAnsibleImple) parseCDIWrapperOutput(output []byte) (jsonData map[string]interface{}, errMsg *string) {
	defer func() {
	...
	case "SUCCESS":
		...
		if len(dataLines) > 0 {
			...
			jsonStr = strings.Replace(jsonStr, "}\"", "}", -1)
			jsonStr = strings.Replace(jsonStr, "None", "null", -1)
			jsonStr = strings.Replace(jsonStr, "False", "false", -1)
			jsonStr = strings.Replace(jsonStr, "True", "true", -1)
			jsonStr = strings.Replace(jsonStr, "'", "\"", -1)
